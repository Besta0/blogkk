 ...
 * Displays comprehensive analytics with real-time updates and data export
      ...
      {displayData.map((item, index) => (
        <div key={index} className="flex-1 flex flex-col items-center gap-1">
          <div
            ...
            className={`w-full ${color} rounded-t-md min-h-[4px] transition-all duration-500`}
          />
        ...
        return (
          <div key={index} className="flex-1 flex flex-col items-center gap-1">
            <div className="w-full flex flex-col-reverse" style={{ height: `${(total / maxValue) * 100}%`, minHeight: '4px' }}>
              <div
                style={{ height: `${viewsHeight > 0 ? (viewsHeight / (total / maxValue * 100)) * 100 : 0}%` }}
                className="w-full bg-blue-500 rounded-b-md transition-all duration-500"
                title={`浏览: ${item.views}`}
              />
              <div
                style={{ height: `${likesHeight > 0 ? (likesHeight / (total / maxValue * 100)) * 100 : 0}%` }}
                className="w-full bg-red-400 transition-all duration-500"
                title={`点赞: ${item.likes}`}
              />
              <div
                style={{ height: `${sharesHeight > 0 ? (sharesHeight / (total / maxValue * 100)) * 100 : 0}%` }}
...
const DonutChart = ({ data }: DonutChartProps) => {
  ...
  const segments = data.map((item) => {
    ...
    const segment = {
      ...
      color: colors[item.device] || '#6B7280',
    };
    currentAngle += angle;
    return segment;
  });
      ...
      {fullData.map((item, index) => (
        <div key={index} className="flex-1 flex flex-col items-center">
          <div
            ...
            className="w-full bg-gradient-to-t from-purple-500 to-pink-400 rounded-t-sm min-h-[2px] transition-all duration-300"
  ...
  return (
    <div className="space-y-4">
      {rankedProjects.map((item, index) => (
        <div
          key={item.projectId}
          className="flex items-center gap-4 p-3 rounded-xl bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all"
        >
          <div
  ...
  return (
    <div className="space-y-3">
      {referrers.slice(0, 5).map((item, index) => (
        <div key={`${item.referrer}-${index}`} className="relative">
          <div className="flex items-center justify-between mb-1">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-white">
            {behavior?.totalSessions ? behavior.totalSessions.toLocaleString() : '0'}
          </p>
        </div>
        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4">
...
const ErrorDisplay = ({ message, onRetry }: ErrorDisplayProps) => (
  <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
    ...
    <button
      onClick={onRetry}
      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white font-medium hover:bg-blue-700 transition-colors"
    >
...
interface ExportButtonProps {
  ...
  onExportInteractions: () => void;
  isExporting: boolean;
}

const ExportButton = ({ onExportPageViews, onExportInteractions, isExporting }: ExportButtonProps) => {
        ...
        <>
          ...
          <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-20">
            <button
              ...
              className="w-full flex items-center gap-2 px-4 py-3 text-left text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-t-lg"
            >
            ...
            <button
              onClick={() => {
                onExportInteractions();
                setShowMenu(false);
              }}
              className="w-full flex items-center gap-2 px-4 py-3 text-left text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-b-lg"
            >
              <FileSpreadsheet className="w-4 h-4" />
              导出项目互动
            </button>
...
export default function AdminAnalyticsDashboard() {
  ...
  // Export handlers
  const handleExportPageViews = useCallback(async () => {
    setIsExporting(true);
    try {
      ...
      console.log('Exporting page views with token:', token.substring(0, 20) + '...');
      
      const params = new URLSearchParams();
      if (startDate) params.set('startDate', startDate);
      if (endDate) params.set('endDate', endDate);
      
      const url = `/api/analytics/export/pageviews${params.toString() ? `?${params.toString()}` : ''}`;
      ...
      a.href = blobUrl;
      a.download = `pageviews_${startDate}_${endDate}.csv`;
  ...
  const handleExportInteractions = useCallback(async () => {
    ...
    try {
      ...
      console.log('Exporting interactions with token:', token.substring(0, 20) + '...');
      
      const params = new URLSearchParams();
      if (startDate) params.set('startDate', startDate);
      if (endDate) params.set('endDate', endDate);
      
      const url = `/api/analytics/export/interactions${params.toString() ? `?${params.toString()}` : ''}`;
      console.log('Export URL:', url);
      ...
      const a = document.createElement('a');
      a.href = blobUrl;
      a.download = `interactions_${startDate}_${endDate}.csv`;
      document.body.appendChild(a);
      a.click();
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-800 dark:text-white">数据分析</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">查看网站访问统计和用户行为</p>
          </div>
        </div>
        <DashboardSkeleton />
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          />
          <ExportButton
            onExportPageViews={handleExportPageViews}
            onExportInteractions={handleExportInteractions}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title="总访问量"
          value={pageViews?.totalViews ? pageViews.totalViews.toLocaleString() : '0'}
          icon={<Eye className="w-6 h-6" />}
          color="blue"
        />
        <StatCard
          title="独立访客"
          value={pageViews?.uniqueVisitors ? pageViews.uniqueVisitors.toLocaleString() : '0'}
          icon={<Users className="w-6 h-6" />}
          color="green"
        />
        <StatCard
          title="24小时访问"
          value={realTimeData?.viewsLast24h ? realTimeData.viewsLast24h.toLocaleString() : '0'}
          icon={<Clock className="w-6 h-6" />}
          color="purple"
        />
        <StatCard
          title="实时访问"
          value={realTimeData?.viewsLastHour ? realTimeData.viewsLastHour.toLocaleString() : '0'}
          subtitle="最近1小时"
